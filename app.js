document.addEventListener('DOMContentLoaded', async () => {
  ...
  const workouts = {
    beginner:{
      Monday:['10min jog+leg swings','4×30s shuttles','3×15 lunges','5min footwork'],
      Tuesday:['3×15 squats','3×10 push-ups','3×20s plank','5min split-step'],
      Wednesday:['5×30s shuffles','3×12 deadlifts','5min hopscotch'],
      Thursday:['3×10 rows','3×12 press','3×20 Russian twists'],
      Friday:['10min run','4×30s high knees','30 net/clear swings'],
      Saturday:['4×5min rallies','10min stretch']
    },
    intermediate:{
      Monday:['15min intervals','5×40m sprints','5min cross-court clears'],
      Tuesday:['4×8 goblet squats','4×10 rows','3×20 bicycle twists'],
      Wednesday:['3×8 box jumps','5min ladder drill','3×15 lateral bounds'],
      Thursday:['4×8 pull-ups','4×10 dips','3×30s side-plank'],
      Friday:['10min tempo run','3×20 fly-runs','30 drives'],
      Saturday:['30min multi-shuttle','15min tactics','10min stretch']
    },
    advanced:{
      Monday:['20min intervals','6×50m sprints','4×8 eight-corner footwork'],
      Tuesday:['5×5 back squats','4×6 deadlifts','4×8 weighted lunges','4×15 wipers'],
      Wednesday:['4×6 depth jumps','10min advanced ladder','4×8 bounding'],
      Thursday:['5×5 push-press','4×8 pull-ups','4×10 dips','3×60s hollow hold'],
      Friday:['10min run+5×20 fly-runs','30 flick returns','30 smash drills'],
      Saturday:['Best-of-3 match','15min cooldown']
    }
  };
  ...
  try {
    ...
    appendChat('ai','Coach loaded! Ask away or analyze a clip. 💪');
  } catch(e) {
    console.error(e);
    appendChat('ai','⚠️ Coach unavailable.');
  }

  // Conversation memory
  const systemPrompt =
    "You are a professional badminton coach. Speak casually, give clear drills, and add motivational emojis.";
  ...
  analyzeBtn.addEventListener('click', async () => {
    const vid = document.getElementById('game-video'),
          ...
          out = document.getElementById('analysis-result');
    ...
    const poses = await detector.estimatePoses(vid).catch(()=>[]);
    ...
    conversation += `\nUser: ${summary}\nCoach:`;
    const reply = await bot.chat(conversation);
    conversation += ` ${reply}`;
    const last = document.querySelector('.chat-message.ai:last-child');
